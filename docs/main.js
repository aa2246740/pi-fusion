(() => {
  ...
  const i18n = {
    en: {
      ...
      benchHeading: "Scored DRACO validation that clears the Fusion API budget baseline.",
      benchIntro: "Pi Fusion is not just a demo of multi-model prompting. We tested it on the same DRACO 10-case benchmark protocol used to evaluate Fusion API budget mode: prompt-only generation first, scorer-only rubric access after answers were sealed. Pi Fusion reached 66.40 in full validation and 66.20 in the latest repeat validation, above the 64.70 budget baseline, with all 10 cases completed and no judge failures.",
      metricPeak: "Best DRACO full10",
      ...
      verifyBaselineLabel: "Baseline",
      verifyBaselineValue: "Fusion API budget, 64.70",
      ...
      benchFootnote: "DRACO-verified here means completed scored runs under this protocol. It is not a claim that Pi Fusion beats every Fusion API mode. Generation used sanitized prompt-only case files; answer/rubric/scoring artifacts were available only to the post-generation scorer.",
    ...
    zh: {
      ...
      benchHeading: "有评分的 DRACO validation，超过 Fusion API budget baseline。",
      benchIntro: "Pi Fusion 不只是 multi-model prompting 的 demo。我们在同一个用于评估 Fusion API budget mode 的 DRACO 10-case benchmark protocol 上测试了它：先进行 prompt-only generation，答案 sealed 之后 scorer 才加载 rubric。Pi Fusion full validation 达到 66.40，latest repeat validation 达到 66.20，高于 64.70 的 budget baseline，10 个 case 全部完成且没有 judge failure。",
      metricPeak: "最佳 DRACO full10",
      ...
      rowBest: "Pi Fusion best validation",
      rowLatest: "Pi Fusion latest validation",
      benchFootnote: "这里的 DRACO-verified 指的是在该 protocol 下完成 scored runs；不是说 Pi Fusion 超过所有 Fusion API modes。生成阶段只使用 sanitized prompt-only case files；answer/rubric/scoring artifacts 只提供给 generation 之后的 scorer。",
      safetyEyebrow: "安全与控制",
